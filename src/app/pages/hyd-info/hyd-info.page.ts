...
export class HydInfoPage extends Baseui implements OnInit {
  ...
  getStations() {
    ...
    this.httpService.getRealStaion({"sttp":"ZQ"}).then(res => {
      ...
      this.dxstation =this.stations.filter(function(item){
        return item.stcd == '60103400'||item.stcd == '60113500'
      });
  ...
  getStationList(event: {
    ...
    if(event.value.length>2||event.value.length<1){
       super.showToast(this.toastCtrl, '只能选择2~3个站');
       return false
    }else{
  ...
  getTab1Data() {
    ...
    if (this.selectTab == "tab1") {
      ...
      if(this.dxstation.length>2||this.dxstation.length<1){
        super.showToast(this.toastCtrl, '只能选择2~3个站');
        return false
     }else{
